...
export default function QuizSetupForm() {
  ...
  async function handleGenerate() {
    setError(null)
    if (!selectedTopics.length) return setError("Pick at least one topic.")
    if (!types.length) return setError("Select at least one question type.")

    const payload: QuizPayload = {
      subjectId, topics: selectedTopics, difficulty, questionTypes: types, numQuestions
    }
    ...
    try {
      ...
      if (!data.quiz?.length) throw new Error("Empty quiz.")

      sessionStorage.setItem("active_quiz", JSON.stringify({
        subjectId, meta: { topics: selectedTopics, difficulty, types, numQuestions }, quiz: data.quiz
      }))
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <select className="w-full border rounded-md h-10 px-3 bg-background"
            value={subjectId} onChange={(e)=>setSubjectId(e.target.value as SubjectId)}>
        ...
        <div className="space-y-2">
          ...
          <Input type="number" min={3} max={30}
            value={numQuestions} onChange={(e)=>setNumQuestions(Math.max(3, Math.min(30, Number(e.target.value))))}/>
        </div>
